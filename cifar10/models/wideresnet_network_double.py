...
class NetworkBlock(nn.Module):
    def __init__(self, N, ni, nf, block, stride, drop_p):
        super().__init__()
        self.layer = nn.Sequential(*[block(ni if i == 0 else nf, nf, stride if i == 0 else 1, drop_p) for i in range(N)])
        
    def forward(self, x): return self.layer(x)

def _make_group(N, ni, nf, block, stride, drop_p):
    print('Making group')
    return nn.Sequential(*[block(ni if i == 0 else nf, nf, stride if i == 0 else 1, drop_p) for i in range(N)])
...
class WideResNet(nn.Module):
    def __init__(self, num_groups, N, num_classes, k=1, drop_p=0.0, start_nf=16):
        ...
        for i in range(num_groups):
            layers.append(NetworkBlock(N, n_channels[i], n_channels[i+1], BasicBlock, (1 if i==0 else 2), drop_p))
